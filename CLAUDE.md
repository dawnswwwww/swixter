...
- **`.github/workflows/release.yml`** (generated by cargo-dist — DO NOT hand-edit; adjust `[workspace.metadata.dist]` in `packages/cli/Cargo.toml` and re-run `cargo dist generate`): builds 7 targets, creates the GitHub Release with changelog + checksums, publishes shell/powershell installers, the npm package, and the Homebrew formula.
