...
${pc.bold("Commands:")}
  ...
  ${pc.cyan("run [options]")}       Run ${CODER_CONFIG.displayName} with current profile
...
${pc.bold("Examples:")}
  ...
  ${pc.dim("# Switch profile")}
  ${pc.green(`swixter ${CODER_NAME} switch my-config`)}

  ${pc.dim("# Apply profile to ${CODER_CONFIG.displayName}")}
  ${pc.green(`swixter ${CODER_NAME} apply`)}

  ${pc.dim("# Run ${CODER_CONFIG.displayName} with current profile")}
  ${pc.green(`swixter ${CODER_NAME} run`)}
  ...
  ${pc.green(`swixter ${CODER_NAME} run --print "What is 2+2?"`)}
`);
}
